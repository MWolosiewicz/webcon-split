...
Write-Host "Wersja pakietu: $version (BPS $bpsLabel)"

dotnet build (Join-Path $projectDir "WebconPdfSplitterAction.csproj") -c Release -p:Version=$version -p:AssemblyVersion=$version -p:BpsSdk=$Sdk
